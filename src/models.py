...
class PaperAnalysis(BaseModel):
    ...
    filename: Optional[str] = Field(None, description="Original PDF filename")
    ...
    has_conflict_of_interest: Optional[bool] = Field(
        None,
        description="True if industry funding/COI detected, False if explicitly declared none, None if not reported",
    )
    funding_source: Optional[str] = Field(None, description="Who funded the study?")
    ...
    primary_outcome: Optional[str] = Field(
        None,
        description="Primary outcome reported by authors, or None if not clearly stated",
    )
    ...
    endpoints: Optional[str] = Field(
        None,
        description="Surrogate markers (e.g. Cholesterol) vs Clinical Endpoints (e.g. Heart Attack)",
    )
    ...
    objective: Optional[str] = Field(
        None, description="Research objective or aim of the study"
    )
    methodology_summary: Optional[str] = Field(
        None, description="Brief summary of methodology used"
    )
    conclusions: Optional[str] = Field(
        None, description="Main conclusions of the study"
    )
    ...
    trust_score: Optional[int] = Field(
        ...
        ge=1,
        le=10,
        description="Model-derived trust score (1–10), None if insufficient evidence",
    )
